...
class RingBuffer:
    def __init__(self, capacity, max_epsiode_len, data_dim):
        ...
        self.max_epsiode_len = max_epsiode_len #最多走多少步
    ...
    def sample_transition_index(self, batch_size, max_past=None):
        ...
        #timesteps = np.random.randint(25, size=batch_size) % lengths ##走的最大的步数max_epsiode_len
        timesteps = np.random.randint(self.max_epsiode_len, size=batch_size) % lengths
...
class ReplayMemory:
    ...
    def add(self, episode_num, obs, action, reward, next_obs, done):
        ...
        for buffer, data in zip(self.buffers.values(), [obs, action, reward, next_obs, done]):
            buffer.store(episode_num, data) ##episode_num 是 global
